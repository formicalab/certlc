...
7. **Grant CA Permissions**:
  - On every certificate template used by CertLC, grant the hybrid worker VM's computer account **Read** and **Enroll** permissions
  - On the issuing CA, open **Certification Authority**, select **Properties > Security**, and grant the hybrid worker VM's computer account **Issue and Manage Certificates**. Revocation uses the CA administration interface (`ICertAdmin::RevokeCertificate`); template **Enroll** permission alone does not authorize it
  - Grant **Manage CA** only if the worker must change CA configuration. CertLC does not require it for enrollment or revocation
